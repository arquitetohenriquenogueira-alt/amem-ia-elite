...
const EMPATIA_MAP = [
    { keywords: ['triste', 'chorar', 'sozinho', 'solidão', 'desespero'], response: "Eu sinto a sua dor, amado. Saiba que Jesus chorou conosco e Ele entende cada lágrima sua. Você não está sozinho, eu estou aqui para orar com você agora." },
    { keywords: ['desemprego', 'trabalho', 'contas', 'dinheiro', 'crise'], response: "Entendo a preocupação que pesa em seus ombros. A incerteza financeira é uma prova difícil, mas o nosso Deus é Jeová Jireh, o Senhor que provê. Vamos descansar na promessa da provisão divina." },
    { keywords: ['doença', 'dor', 'enfermidade', 'cancer', 'saúde'], response: "Sinto muito que você esteja enfrentando essa batalha na saúde. Meu coração se inclina em oração por você agora. Creia que para o nosso Deus, não há diagnóstico impossível." }
];
...
function addXP(amount) {
    ...
    if (novoNivel > memory.nivel) {
        alert(`Glória a Deus! Você subiu para o Nível ${novoNivel}: ${NIVEIS_NOMES[novoNivel - 1]}`);
...
function falarMensagem(texto) {
    ...
    const maleVoice = voices.find(v => v.name.includes('Google português do Brasil') || v.name.toLowerCase().includes('male') || v.lang === 'pt-BR');
